...
wheel_base = 10.4 #if undershoot, decrease
#prev 10.5
wheel_diameter = 1.215 #if undershoot, increase 
#prev 1.2

x_curr = -5.6875 - wheel_base/2
y_curr = 0
...
while True:
    ...
    elapsed_time = current_time - prev_time  # Compute actual time step
    
    light_val = light_sensor.lux  # Ensure this updates correctly

    #line follow
    line_follower.update_velocities(light_val)

    #update odometry, get angle, get tof data
    odometry.update(d_t)  # Use actual elapsed time
    prev_time = current_time  # Update previous time

    odometry.print_phi()
    time.sleep(d_t)  # Keep this but monitor actual execution speed
