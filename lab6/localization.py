...
class Localization: 
    ...
    def __init__(self, regions_per_sector:int, block_threshold_upper: float, block_threshold_lower: float, blocks_map: list, goal_region:int):
        ...
        self.goal_region = goal_region * self.regions_per_sector
    ...
    def integrated_update(self, distance_reading, current_angle, sigma_region=5, sigma_sensor=2, d_expected=23):
        ...
        for region in range(self.total_regions):
            likelihood = self.sensor_model(distance_reading, d_expected, sigma_sensor)
           
            updated_bel[region] = predicted_bel[region] * likelihood[0]
